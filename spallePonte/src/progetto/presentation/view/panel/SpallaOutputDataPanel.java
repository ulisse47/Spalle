 ...
 */
public class SpallaOutputDataPanel extends AbstractInputPanel {

	private static SpallaOutputDataPanel panel;

	public static synchronized SpallaOutputDataPanel getInstance() {
		if (panel == null) {
			panel = new SpallaOutputDataPanel();
		}
		return panel;
	}
...
//		addInputField(pTerrenoCLS, new DefaultDoubleField(width), 1, 0, "y_verifica", "coordinata y sezione di verifica(cm)");		
	...
	public void refreshView() {
		...
		try {
			InputComponent comp;
/*			double[] sigmaWx = del.get_sigma_wx_wy();
			comp = (InputComponent) outputs.get("sigma");
			comp.setValue(new Double(sigmaWx[0]));
			comp = (InputComponent) outputs.get("wx");
			comp.setValue(new Double(sigmaWx[1]));
			comp = (InputComponent) outputs.get("wy");
			comp.setValue(new Double(sigmaWx[2]));
*/	
